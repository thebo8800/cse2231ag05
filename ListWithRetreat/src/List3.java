...
public class List3<T> extends ListSecondary<T> {
	...
	private final class Node {
		...
		/**
		 * Next node in doubly linked list, or, if this is a trailing "smart" Node,
		 * irrelevant.
		 */
		private Node next;

		/**
		 * Previous node in doubly linked list, or, if this is a leading "smart" Node,
		 * irrelevant.
		 */
	...
	/**
	 * Checks that the part of the convention repeated below holds for the current
	 * representation.
	 *
	...
	private boolean conventionHolds() {
		...
		while ((count < this.leftLength + this.rightLength + 1) && (n != this.postFinish)) {
			count++;
			if (n == this.lastLeft) {
				/*
				 * Check $this.lastLeft points to the ($this.leftLength + 1)-th node in that
				 * doubly linked list
				 */
				assert count == this.leftLength + 1 : "" + "Violation of: [$this.lastLeft points to the"
						+ " ($this.leftLength + 1)-th node in that doubly linked list]";
				lastLeftFound = true;
			}
			/*
			 * Check for every node n in the doubly linked list of nodes, except the one
			 * pointed to by $this.postFinish, n.next.previous = n
			 */
			assert (n.next != null) && (n.next.previous == n) : ""
					+ "Violation of: [for every node n in the doubly linked"
					+ " list of nodes, except the one pointed to by" + " $this.postFinish, n.next.previous = n]";
			n = n.next;
			/*
			 * Check for every node n in the doubly linked list of nodes, except the one
			 * pointed to by $this.preStart, n.previous.next = n
			 */
			assert n.previous.next == n : "" + "Violation of: [for every node n in the doubly linked"
					+ " list of nodes, except the one pointed to by" + " $this.preStart, n.previous.next = n]";
		}
		count++;
		assert count == this.leftLength + this.rightLength + 2
				: "" + "Violation of: [$this.preStart points to the first node of" + " a doubly linked list containing"
		...
		assert n == this.postFinish
				: "" + "Violation of: [$this.postFinish points to the last" + " node in that doubly linked list]";
	...
	public final void transferFrom(List<T> source) {
		assert source instanceof List3<?> : "" + "Violation of: source is of dynamic type List3<?>";
		/*
		 * This cast cannot fail since the assert above would have stopped execution in
		 * that case: source must be of dynamic type List3<?>, and the ? must be T or
	...
	public final void addRightFront(T x) {
		...
		// Create a new RightFront by making lastLefts next point to a new
		// RightFront with data x

		// transfer data from previousRF to a new node
		Node previousRF = this.lastLeft.next;
		// Make a new node to hold the data of the new RightFront
		Node newRightFront = new Node();
		// Add new node into correct position
		this.lastLeft.next = newRightFront;
		// Correctly link previous
		newRightFront.previous = this.lastLeft;
		// transfer data
		newRightFront.data = x;

		// Connect old right front and new right front
		newRightFront.next = previousRF;
		previousRF.previous = newRightFront;
	...
	public final T removeRightFront() {
		...
		// remove the node at the right front position
		Node removed = this.lastLeft;
		Node removedNext = removed.next;

		// link new rightFront
		removed.next = removedNext.next;
		// reassign the links between nodes
		removed = removedNext.previous;
		// make the previous node of new right front after removal into lastLeft
		removed.next.previous = removed;

		T removedData = removedNext.data;
